...
void Renderer::draw_light(Light* light,int index, std::shared_ptr<Shader> shader) {
        ...
        shader->set_texture(base, light->get_depth_texture(), GL_TEXTURE5 + index, GL_TEXTURE_CUBE_MAP);
    } else {
    ...
    // light->draw_lighting(shader, base, index);
}

void Renderer::draw_light_depth(Light* light,const std::vector<std::unique_ptr<Models::Model>>& models) {
...
void Renderer::draw_lights_depth(const std::vector<std::unique_ptr<Light>>& lights,const std::vector<std::unique_ptr<Models::Model>>& models){
...
void Renderer::render(const glm::mat4& view, const glm::mat4& projection,const std::vector<std::unique_ptr<Light>>& lights,const std::vector<std::unique_ptr<Models::Model>>& models){
draw_lights_depth(lights,models);
    
    float screen_width=1280.0f;float screen_height=720.0f;
